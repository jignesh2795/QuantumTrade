...
def generate_mock_trades(count: int) -> list:
    ...
    for i in range(count):
        trade = {
            ...
            - timedelta(minutes=random.randint(0, 1440)),  # Up to 24 hours ago
        }
        trades.append(trade)
...
def load_mock_data():
    ...
    mock_executions = generate_mock_strategy_executions(
        MOCK_DATA_COUNT // 5
    )  # Fewer execution entries
