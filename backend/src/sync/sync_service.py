...
def sync_table(table_name: str) -> bool:
    ...
    if success:
        logger.info(f"Successfully synced {len(data)} records to local table {table_name}")
    else:
...
def sync_tables():
    ...
    print(f"✅ Sync complete: {success_count}/{len(tables_to_sync)} tables synced successfully")
